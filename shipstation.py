...
def fetch_shipstation_products(session):
    ...
    # Define the productCategoryId for "SchwagShop"
    # product_category_id = 15456  # Replace with the actual ID if different

    # Include the productCategoryId in the request URL
    if product_category_id:
        request_url = f"{base_url}/products?productCategoryId={product_category_id}"
    else:
        request_url = f"{base_url}/products"

    print(f"Getting ShipStation products from {request_url} with key={ss_api_key} and secret={ss_api_secret}")
        ...
        for product in shipstation_products:
            if product.get("active", None) is True:
                transformed_product = {
                    ...
                    "price": 100,  # Static price for this example
                    "sku": product.get("sku", ""),
                    "thumbnailURL": product.get("thumbnailURL", ""),
                }
                print(f"Transformed product: {transformed_product}")
